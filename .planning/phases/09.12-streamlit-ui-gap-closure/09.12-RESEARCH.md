...
def _burn_captions(
    ...
    *,
    ...
    ...
    # None = "Auto" — derive from platform spec at render time.
    # Explicit "16:9"/"9:16"/"1:1" overrides the spec-derived value.
    aspect_ratio = (
...
# None = "Auto — match export target" (infer from spec.aspect_ratio at render time)
ASPECT_RATIO_OPTIONS = [None, "16:9", "9:16", "1:1"]
ASPECT_RATIO_LABELS = ["Auto (match export target)", "16:9 (Landscape)", "9:16 (Vertical)", "1:1 (Square)"]
...
caption_aspect_ratio = st.selectbox(
    ...
    help="Auto derives safe-zone from platform spec. Override to force a specific template.",
)
# None stored in ReviewDecisions means "derive from spec at render time"
```
...
## Resolved Decisions (user-confirmed 2026-02-24)

1. **`caption_aspect_ratio = None` means "infer from platform spec at render time."**
   The render stage already has the platform spec for each export — Instagram gets 1:1, TikTok/Reels gets 9:16, YouTube gets 16:9 automatically. The UI selectbox offers `None` (labeled "Auto — match export target") as the default option, plus "16:9", "9:16", "1:1" as explicit overrides for when the user wants to force a specific template regardless of platform. Do NOT hardcode 16:9 as the default — that would silently produce wrong safe-zones for vertical/square exports.

2. **Provider selectbox is session-only + informational.**
   Provider selection is a config-time decision — the analyze stage reads `config.models.provider` at startup, before review decisions exist. It is NOT a per-job `ReviewDecisions` field. The UI shows:
   - Current provider + key status (read-only, informational)
   - `ANTHROPIC_API_KEY` status alongside Gemini/Kimi
   - No config.yaml write-back (permission complexity, affects all future jobs)
   - Visible note: "To change provider, set `models.provider` in config.yaml or restart with `MODELS__PROVIDER=claude`"

3. **GAP-5B (per-job stinger path override) → P3 stretch goal.** Include as optional final task that can be skipped.

4. **GAP-6B (platform override interactive edit) → P3 stretch goal.** Include as optional final task that can be skipped. P0/P1/P2 gaps are what matter for correctness.
