        ...
        try:
            ...
            if not fillers:
                ...
                with st.expander(f"View {len(fillers)} filler words"):
                    approved_fillers = decisions.approved_filler_cuts if decisions else list(range(len(fillers)))
...
def render_thumbnail_selector(job: Job, job_dir: Path) -> None:
    ...
    for row_start in range(0, len(thumbnails), cols_per_row):
        ...
        for col_idx, thumb_idx in enumerate(range(row_start, min(row_start + cols_per_row, len(thumbnails)))):
            thumb = thumbnails[thumb_idx]
            with cols[col_idx]:
                # Thumbnail card
                is_selected = thumb_idx == selected_idx
                border_style = "border: 3px solid #00ff00;" if is_selected else "border: 1px solid #ccc;"
...
def render_export_panel(job: Job, job_dir: Path) -> None:
    ...
    with col1:
        if not review_complete:
            ...
            if st.button("🎬 Export Now", key="export_now"):
                update_export_platforms(job_dir, new_selected)
                run_stage(job, "render")
...
def main() -> None:
    ...
    # Sidebar navigation
    with st.sidebar:
        st.image("https://www.shutterstock.com/shutterstock/photos/2047266761/display_1500/stock-vector-banner-template-for-podcast-show-microphone-headphones-text-and-sound-wave-trend-vector-design-2047266761.jpg", use_container_width=True)
        ...
        st.divider()
        st.caption(f"v0.1.0 | Streamlit UI")
...
def render_settings() -> None:
    ...
    with st.expander("🤖 AI Models"):
        ...
        st.text_input("Fallback Provider", value=config.models.fallback_provider or "None", disabled=True)
