...
class ViralClipDetector:
    ...
    EMOTIONAL_WORDS = {
        "high_positive": ["amazing", "incredible", "breakthrough", "revolutionary", "life-changing", "mind-blowing"],
    ...
    def __init__(self) -> None:
        ...
        self._compiled_hook_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.HOOK_PATTERNS
        ]
            ...
            for emotion_type, words in self.EMOTIONAL_WORDS.items():
                for word in words:
                    if word in text_lower:
                        strength = 0.9 if emotion_type in ["high_positive", "high_negative"] else 0.7
                        signals.append(EngagementSignal(
                            timestamp_seconds=start,
    ...
    def score_clip(
        ...
        clip_signals = [
            s for s in signals
            if start <= s.timestamp_seconds <= end
        ]
        ...
        reasons = self._generate_reasons(hook_score, emotional_score, shareability_score, clip_signals)
    ...
    def _calculate_hook_score(
        ...
        early_hooks = [
            s for s in signals
            if s.signal_type == "hook" and s.timestamp_seconds - clip_start < 10
        ]
    ...
    def _calculate_emotional_score(
        ...
        emotional_signals = [
            s for s in signals
            if s.signal_type in ["emotional_peak", "dramatic_pause"]
        ]
    ...
    def suggest_clips(
        ...
        for hook in hooks:
            ...
            punchlines_after = [
                s for s in signals
                if s.signal_type == "punchline" and s.timestamp_seconds > start
            ]
        ...
        for candidate in candidates:
            if not any(
                self._clips_overlap(candidate, existing)
    ...
    def _clips_overlap(
        ...
        start1, end1 = clip1["start_seconds"], clip1["end_seconds"]
        start2, end2 = clip2["start_seconds"], clip2["end_seconds"]
